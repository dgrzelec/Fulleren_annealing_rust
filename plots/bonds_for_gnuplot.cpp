...
int main(int argc, char** argv){
	...
	if (argc == 2) {
		...
		double r_max = 1.3*stod(argv[1]);
		int N = 60;

		const char * fname_out = "plots/bonds.dat";
		const char * fname_in = "plots/atoms.dat";
		// ####################################
		cout << "works" << endl;

		// reading vactor<vector<double>> from file
		std::vector<std::vector<double>> v;
		std::ifstream in( fname_in );
		std::string record;
		...
		// 
		write_polygons_from_atoms(r_max, N, v, fname_out);

	}
	else {
		cout << "Invalid or no arguments given!" << endl;
	}
}
